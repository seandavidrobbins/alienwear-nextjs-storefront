...
export default function displayBlock(block, index) {
  ...
  switch (name) {
    ...
    case 'core/list': {
      ...
      return <BlockList {...attributes} key={index} innerBlocks={innerBlocks} />
    }
    ...
    /* -- ACF BLOCKS -- */
    case 'acf/hero-video': {
      const BlockHeroVideo = dynamic(() =>
        import('components/blocks/ACF/BlockHeroVideo')
      )
      return <BlockHeroVideo {...attributes} key={index} />
    }

    case 'acf/lad-media': {
      const BlockCustomMedia = dynamic(() =>
        import('components/blocks/ACF/BlockCustomMedia')
      )
      return <BlockCustomMedia {...attributes} key={index} />
    }

    case 'acf/lad-contact': {
      const BlockCustomContact = dynamic(() =>
        import('components/blocks/ACF/BlockCustomContact')
      )
      return <BlockCustomContact {...attributes} key={index} />
    }

    case 'acf/lad-about': {
      const BlockCustomAbout = dynamic(() =>
        import('components/blocks/ACF/BlockCustomAbout')
      )
      return <BlockCustomAbout {...attributes} key={index} />
    }

    case 'acf/portfolio-repeater': {
      const BlockPortfolioRepeater = dynamic(() =>
        import('components/blocks/ACF/BlockPortfolioRepeater')
      )
      return <BlockPortfolioRepeater {...attributes} key={index} />
    }
